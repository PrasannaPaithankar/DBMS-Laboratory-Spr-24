...
from .models import (Event, Event_Participant, Notification, Organizer,
                     Participant, Role, Student, Student_Event, Volunteer,
                     food)
...
def create_app():
    ...
    def events():
        if 'role' in session:
            if session['role'] == 'external':
                ...
                user = Student.query.filter_by(Roll=session['user_id']).first()
                user_events = Student_Event.query.filter_by(
                    Roll=user.Roll).all()
                user_volunteers = Volunteer.query.filter_by(
                    Roll=user.Roll).all()
                if user is not None:
        ...
        if request.method == 'POST':
            if request.form['formtype'] == 'mail':
                ...
                return render_template('organizerPanel.html', user=organizer,
                                       events=[Event.query.filter_by(EID=organizer.EID).first()])
    ...
    def sendnotification():
        ...
        if request.method == 'POST':
            ...
            notification = Notification(receiver=Vroll,
                                        message=message,
                                        time=datetime.now(),
    ...
    def volunteerdashboard():
        messages = (
            db_session.query(Student.Name, Student.Roll,
                             Notification.message, Notification.time)
            .join(Student, and_(Notification.sender == Student.Roll,
                                Notification.receiver == session['user_id']))
    ...
    def searchvolunteers():
        volunteers = (db_session.query(Volunteer.Roll, Student.Name,
                                       Student.email, Event.EName)
                      .join(Event, Volunteer.EID == Event.EID)
                      .join(Student, Volunteer.Roll == Student.Roll)
                      .all())

        print(volunteers)
        return render_template('searchvolunteers.html',
                               volunteers=volunteers,
                               user=session['user_id'],
                               role=session['role'])

    @app.route('/sendnotficationvolunteer/<volunteers>',
               methods=['GET', 'POST'])
    def sendnotficationvolunteer(volunteers):
        ...
        if request.method == 'POST':
            ...
            message = request.form['message']
            # Add it to the database
            notification = Notification(receiver=Vroll, message=message,
                                        time=datetime.now(),
                                        sender=session['user_id'])
            db_session.add(notification)
            db_session.commit()
    ...
    def efood():
        if 'role' in session:
            if session['role'] == 'external':
                ...
                print(foodtype)
                foods = db_session.query(
                    food.Name, food.detail).filter_by(type=foodtype).all()
                print(foods)
